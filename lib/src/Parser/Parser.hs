...
module Parser.Parser(parseAbcFile,parseSwfFile,parseAbcAsSwf,parseAnyAsSwf) where
...
import Parser.InsertLabels (insertLabels)
import Codec.Archive.Zip
import System.FilePath
import Parser.BranchRelToAbs (branchRelToAbs)
import Parser.FixJumpDestinations (fixJumps)
import Parser.RenumberBlocks (renumberBlocks)
import Debug.Trace
...
-- Along with parsing the bytecode is exposed to the sequence of
-- transformations implemented by the following functions:
-- (the order of introduction corresponds to the transformation order)
-- 
-- branchRelToAbs :: LabInstruction -> LabInstruction
-- fixJumps       :: BodyInfo       -> BodyInfo
-- insertLabels   :: BodyInfo       -> BodyInfo   
-- renumerBlocks  :: SwfFile        -> SwfFile
