module Util.ProgInfo
  ...
  , allClasses, classQName, spClassQName, isAccessible
  , module Util.Env, module Util.EnvInfo
  , CtxInfo(..), CtxBlockInfo(..), CtxMethodInfo(..), CtxTrait(..), CtxObj(..), CtxParents(..), CtxTraitBind(..)
  , parentsToMethodName
  , PropLookupTable
  , symTbl2PropLookupTbl
  , typeInfo2PropLookupTbl
  ) where
...
-- Symbol tables
-- FIX: add explanatons to the fields of SymbolTable
data SymbolTables = SymbolTables
  ...
  , tablePropLookup :: PropLookupTable -- property lookup table. it's used by boogie
  , graphTypeInfo   :: TpGr
  }
  deriving Show


-- It would be more logical to call NamePool, NamesetsPool and NamespacePool 
-- as NamespacePool, NamespaceSetPool and MultinamePool respectively. 
-- This would correspond to the original pool names introduced in AVM2 spec.
...
type NamePool        = StaticEnv Name       -- namespace     ?
type NamesetsPool    = StaticEnv Nameset    -- namespace_set ?
type NamespacePool   = StaticEnv Namespace  -- multiname     ?
...
data Sig = Sig { sigName   :: !(Maybe StringRef)  -- may be nameless
               , sigReturn :: !NameRef            -- name of return type
...
spClassQName :: SymbolTables -> ClassRef -> Maybe QName
spClassQName tbls ref =
  case clSuper $ lookupClass ref tbls of
    Nothing   -> Nothing
    Just nref -> Just $ toQName tbls nref
...
-- | A table that contains classes with their properties.
-- | It's used for name lookup that is carry out by AVM2 instructions findproperty.
-- | See Boogie translation encoding.

type PropLookupTable = Map QName [QName]

typeInfo2PropLookupTbl :: TypeInfo -> PropLookupTable
typeInfo2PropLookupTbl tpi =
  Map.map (\td -> map tpTrName  $ tpDynTraits td ++ tpStaTraits td) $ unTypeInfo tpi

symTbl2PropLookupTbl :: [SymbolTables] -> PropLookupTable
symTbl2PropLookupTbl tbls = typeInfo2PropLookupTbl $ ( mconcat $ map tableTypeInfo tbls)
-- symTbl2PropLookupTbl tbls =
--   let tpInfos         = map (unTypeInfo . toTypeInfo) tbls
--       colTrNames td   = map tpTrName  $ tpDynTraits td ++ tpStaTraits td
--       disjUnion m1 m2 = if Map.intersection m1 m2 == Map.empty
--                   then Map.union m1 m2
--                   else error "map intersection is not empty"
--       disjUnions      = foldl disjUnion Map.empty
--   in  disjUnions $ map (Map.map colTrNames) tpInfos
