...
import Data.Monoid
import Util.ParamAnalysis (emptyMethodDefInfos, analyzeDefsSwf, analyzeDefsAbc)
import Data.Maybe
...
runIt !mbInj !source !dest = runIt__ myOptions mbInj source dest
   where
   myOptions = defaultOpts { optVerbose      = False
                           , optPrettyLabs   = True
...
runIt__ !myOptions !mbInj !source !dest = do
  ...
  -- let defSites  = emptyMethodDefInfos
  hPutStrLn stderr "analyzed stack parameters"
  ...
  case optSaveCFG opts of
    ...
    Just file | null file -> do hPutStrLn stderr "CFG is outputted"
                                L.writeFile (replaceExtension source ".dot")
                                  $ C.pack
                                  $ exportCFG opts tbls defSites swf
  ...
  case optSaveAst opts of
    Nothing   -> hPutStrLn stderr "AST isn't pretty printted" >> return ()
    Just file | null file -> do hPutStrLn stderr "AST is pretty printted"
                                L.writeFile (replaceExtension source ".pp")
                                  $ renderBytes
                                  $ PT.ppSwf opts tbls defSites swf
              | otherwise -> do hPutStrLn stderr "AST is pretty printted" 
                                L.writeFile file
                                  $ renderBytes
                                  $ PT.ppSwf opts tbls defSites swf
...
-- runItAbc :: Options -> FilePath -> FilePath -> IO ()
-- runItAbc myOptions source dest = do
--   let opts = myOptions {optSourceFile = source}
--   abc <- pAbcFile source
--   hPutStrLn stderr "Parsed abc file"
--   let tblsInitial = symInfoAbc abc
--   hPutStrLn stderr "Extracted symbol info."
--   let defSites = analyzeDefsAbc opts abc
--   hPutStrLn stderr "Analyzed stack parameters."
--   case optSaveAst opts of
--     Nothing   -> hPutStrLn stderr "AST isn't pretty printted" >> return ()
--     Just file | null file -> do hPutStrLn stderr "AST is pretty printted"
--                                 L.writeFile (replaceExtension source ".pp")
--                                   $ renderBytes
--                                   $ PT.ppAbc opts [tbls] defSites swf
--               | otherwise -> do hPutStrLn stderr "AST is pretty printted" 
--                                 L.writeFile file
--                                   $ renderBytes
--                                   $ PT.ppAbc opts [tbls] defSites swf
...
--                                 L.writeFile (replaceExtension source ".bpl")
--                                   $ renderBytes
--                                   $ abcToBoogie opts tbls defSites swf
--               | otherwise -> do hPutStrLn stderr "swf is tanslated to boogie" 
--                                 L.writeFile file
--                                   $ renderBytes
--                                   $ abcToBoogie opts tbls defSites swf   
